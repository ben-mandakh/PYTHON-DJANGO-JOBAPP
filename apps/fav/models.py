...
class BookManager(models.Manager):
    def validator(self, postData):
        ...
        if len(postData['desc']) < 5:
            errors["desc"] = "Desciption should be at least 5 characters"
        return errors
...
class Book(models.Model):
    ...
    uploaded_by = models.ForeignKey(User, related_name="books")          # ONE TO MANY RELATIONS HERE
    users_who_like = models.ManyToManyField(User, related_name="liked_books")     # MANY TO MANY RELATIONS HERE
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    objects = BookManager()